...
import { formatDateTimeRange, buildEventLabels, groupEventsByPeriod } from "@/lib/utils";
import { EVENT_TYPES } from "@/lib/constants";
import Link from "next/link";
import { X, MapPin, Edit2, Trash2, Users, Check, XCircle, ChevronDown, ChevronRight } from "lucide-react";
...
function ScheduleContent() {
  ...
  const upcomingGroups = useMemo(() => groupedEvents.filter((g) => g.label !== "Past"), [groupedEvents]);
  const pastGroup = useMemo(() => groupedEvents.find((g) => g.label === "Past"), [groupedEvents]);
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6">
      ...
      {loading ? (
      ...
      ) : upcomingGroups.length === 0 && pastGroup ? (
        ...
        <div>
          ...
          >
            ...
            Past Events ({pastGroup.events.length})
          </button>
          {showPast && (
            <div className="space-y-3 opacity-70">
              {pastGroup.events.map((event) => renderEventCard(event))}
            </div>
          )}
        </div>
      ) : (
