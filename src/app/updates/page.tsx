...
function UpdatesContent() {
  ...
  const [form, setForm] = useState({ title: "", message: "", sendSms: false });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        await load();
        setShowForm(false);
        setForm({ title: "", message: "", sendSms: false });
        addToast(form.sendSms ? "Update posted & SMS sent" : "Update posted", "success");
      } else {
          ...
          <CardContent className="py-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-sm">Also send SMS to all families</span>
              </label>
